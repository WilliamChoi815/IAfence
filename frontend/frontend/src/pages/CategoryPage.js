...
// Fetch Recent Tournament
const tournamentsRes = await fetch(
`${process.env.REACT_APP_API_URL}/tournament-matches/${
category.charAt(0).toUpperCase() + category.slice(1)
}/`
);
...
(a, b) => new Date(b.tournament_date) - new Date(a.tournament_date)
);
setTournament(sortedTournaments[0]);
}
} catch (error) {
